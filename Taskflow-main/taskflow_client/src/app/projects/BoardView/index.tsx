...
const BoardView = ({ id, setIsModalNewTaskOpen }: BoardProps) => {
  ...
  if (error) {
    return <div>Error occurred while loading tasks</div>;
  }
  ...
  return (
    ...
    <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2 xl:grid-cols-4">
      {Object.values(Status).map((status) => {
        // Using any type to bypass key prop validation
        const TaskColumnElement: any = TaskColumn;
        return (
          <TaskColumnElement
            key={status}
            status={status}
  ...
  return (
    <div
      ...
      {tasks
        ...
        .map((task) => {
          const taskCardProps: TaskCardProps = { task: task };
          // @ts-ignore
          return <TaskCard key={task.id} {...taskCardProps} />;
        })}
    </div>
...
const getTaskTagSplit = (task: TaskType) => task.tags ? task.tags.split(',') : [];

const getFormattedStartDate = (task: TaskType) => task.startDate ? format(new Date(task.startDate), 'P') : '';
const getFormattedDueDate = (task: TaskType) => task.dueDate ? format(new Date(task.dueDate), 'P') : '';

const getNumberOfComments = (task: TaskType) => (task.comments ? task.comments.length : 0);

const getTaskPriority = (task: TaskType) => (
  <div 
    className={`rounded-full px-2 py-1 text-xs font-semibold ${task.priority === 'Urgent' ? 'bg-red-200 text-red-700' :
      task.priority === 'High' ? 'bg-yellow-200 text-yellow-700' :
        task.priority === 'Medium' ? 'bg-green-200 text-green-700' :
          'bg-gray-200 text-gray-700'
      }`}>
